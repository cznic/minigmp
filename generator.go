...
`
	...
	prologue = `// Copyright 1991-1997, 1999-2016 Free Software Foundation, Inc.  All rights
...
func lib() {
	const tweak = `
	crt.Xfprintf(tls, (*crt.XFILE)(Xstderr), str(158), unsafe.Pointer(unsafe.Pointer(_msg)))
	crt.Xabort(tls)
`
	repo := findRepo(repo)
	if repo == "" {
	...
	if bytes.Index(b2, []byte(tweak)) < 0 {
		log.Fatal("internal error")
	}

	b2 = bytes.Replace(b2, []byte(tweak), []byte(" panic(crt.GoString(_msg)) "), 1)
